...
/// Bind mounts a directory in an unshared mount namespace
///
/// In order to do so, enters a user namespace if required not already root.
///
/// On success, this function returns but potentially from the context of a
/// child process. The parent process will eventually exit with the same exit
/// code as the child process, and never return.
pub fn with_optional_bind_mount<F, R>(source: &Path, target: &Path, f: F) -> R
where
    F: FnOnce(&Path, &Path) -> R,
{
    ...
    // Unshare user & mount namespaces if possible, and if not, run the callback
    // with the original source directory
    if unshare(CloneFlags::CLONE_NEWUSER | CloneFlags::CLONE_NEWNS).is_err() {
        eprintln!("namespace unsharing unsupported or blocked");
        return f(source, source);
    }

    // Remap own uid & gid
    File::create("/proc/self/setgroups")
        .expect("failed to create setgroups")
        .write_all(b"deny")
        .expect("Failed to deny setgroups");
    File::create("/proc/self/uid_map")
        .expect("failed to open uid_map")
        .write_all(format!("{uid} {uid} 1").as_bytes())
        .expect("failed to remap uid");
    File::create("/proc/self/gid_map")
        .expect("failed to open gid_map")
        .write_all(format!("{gid} {gid} 1").as_bytes())
        .expect("failed to remap gid");

    // Fork and continue running from the child in order to get into the
    // unshared process namespace
    // This is unsafe to do in multi-threaded programs, but this code is run
    // from the constructor so it's assured to be single-threaded.
    unsafe {
        if let unistd::ForkResult::Parent { child } = unistd::fork().expect("failed to fork") {
            loop {
                match wait::waitpid(child, None) {
                    Ok(wait::WaitStatus::Exited(_pid, code)) => libc::_exit(code),
                    Ok(_) | Err(errno::Errno::EINTR) => {}
                    Err(e) => {
                        eprintln!("waitpid returned {e:?}");
                        libc::_exit(e as _);
                    }
                }
            }
    ...
    .expect("failed to bind mount {source} to {target}");

    let ret = f(source, target);

    // Clean up
    mount::umount(target).expect("failed to unmount {target}");
...
fn main() {
    ...
    if let Some(lib_dir) = std::env::var_os("VECTORSCAN_LIB_DIR") {
        ...
        // the same, mount bind the out directory to /tmp and use that as the
        // directory which contains the cmake build sub-directory.
        with_optional_bind_mount(
            &out_dir,
            Path::new("/var"),
