...
fn with_bind_mount_impl<F, R>(source: &Path, target: &Path, f: &F) -> Result<R>
where
    F: Fn(&Path, &Path) -> R,
{
    // Get real uid & gid
    ...
    unsafe {
        if let unistd::ForkResult::Parent { child } = unistd::fork().context("fork")? {
            loop {
...
where
    F: Fn(&Path, &Path) -> R,
{
    match with_bind_mount_impl(source, target, &f) {
        Err(e) => {
